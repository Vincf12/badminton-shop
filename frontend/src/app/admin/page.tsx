...
const navItems = [
  ...
  { label: "Banner", icon: Boxes, href: "/admin/banners" },
...
function NewUsersList({ users }: { users: NewUserStat[] }) {
  return (
    <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
      <div className="mb-5 flex items-center justify-between gap-4">
        <div>
          <h2 className="text-lg font-black text-slate-950 dark:text-white">Khách hàng mới</h2>
          <p className="text-sm text-slate-500 dark:text-slate-400">Tài khoản vừa tham gia hệ thống.</p>
        </div>
      ...
      <div className="space-y-3">
        {users.length === 0 ? (
          ...
          users.slice(0, 6).map((item) => (
            <div
              key={item.userId}
              className="flex items-center justify-between gap-4 rounded-2xl border border-slate-100 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-950/60"
            >
              <div className="flex min-w-0 items-center gap-3">
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-emerald-100 text-sm font-black text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-300">
                  {(item.fullName || item.email || "U").charAt(0).toUpperCase()}
                </div>
                <div className="min-w-0">
                  <p className="truncate font-black text-slate-900 dark:text-white">{item.fullName || item.email}</p>
              ...
              <div className="shrink-0 text-right">
                <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-black text-slate-700 ring-1 ring-slate-200 dark:bg-slate-800 dark:text-slate-200 dark:ring-slate-700">
                  {item.role || "user"}
                </span>
...
function QuickActions() {
  const actions = [
    ...
    { label: "Quản lý coupon", icon: BadgePercent, href: "/admin/coupons" },
    { label: "Xử lý đơn hàng", icon: ClipboardList, href: "/admin/orders" },
    { label: "Quản lý thương hiệu", icon: Building2, href: "/admin/brands" },
  ];
...
export default function DashboardPage() {
  ...
  const canViewDashboard = role === "admin" || role === "staff";
            ...
            <main className="space-y-6 px-4 py-6 sm:px-6 lg:px-8">
              ...
              {loading ? (
                ...
                <>
                  ...
                  <section className="grid gap-6 xl:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)]">
                    <NewUsersList users={data.newUsers} />
                    <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                      <div className="mb-5">
                        <h2 className="text-lg font-black text-slate-950 dark:text-white">API đã kết nối</h2>
                        <p className="text-sm text-slate-500 dark:text-slate-400">Trạng thái các nguồn dữ liệu quản trị trên màn hình này.</p>
                      </div>
                      <div className="grid gap-3 sm:grid-cols-2">
                        {[
                          ...
                          <div
                            key={item}
                            className="flex items-center gap-3 rounded-2xl border border-emerald-100 bg-emerald-50 px-4 py-3 text-sm font-black text-emerald-800 dark:border-emerald-500/20 dark:bg-emerald-500/10 dark:text-emerald-300"
                          >
